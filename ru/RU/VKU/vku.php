...
<input  id="ses_name" type="hidden"  value = "<?php  echo $_SESSION['login'];?>" />
     ...
     <h2 >Регистрационная карточка граждан Республики Узбекистан <span>временно</span> проживающих за границей</h2>
            <br/>
     <div id="wrapper1">
...
<label for="tab-4" id="tab-label-4">Адрес постоянного проживания в РУ </label>
...
<article  class="tab-5"> <?php include "vku/vku5.php"; ?>
  ...
  <div class="Yuklash"><span>Копия паспорта&nbsp&nbsp&nbsp&nbsp&nbsp</span>
  <div class="Copy_Pas">
  <label>Загрузка...
...
<div class="Yuklash1"><span>Копия справки с места регистрации на территории страны пребывания</span>
 <div class="Copy_Mal">
   <label>&nbsp&nbsp&nbspЗагрузка...
...
<button id="registerButton2" type="submit" >Отправка (печать) анкеты</button>
 </div>
                     <div id="anketa_res">
                       <br/>
                     <br/>
     <p >
  Для просмотра ответа на запрос  введите<span> ID(>= 10 символов)</span>
      </p> 
...
<div class="popup">
  <div class="popup_title">
    Результат <span class="closer">X</span>
  </div>
  <div class="popup_content">
    <p id="z_id"></p>
    <p id="javob"> </p>
    <div align="center">
    <button class="but" ><span>Редактировать</span></button>
    </div>
...
$('#registerButton2').attr('disabled', true);
  ...
  //
  	...
  	if((ext == "jpg")||(ext =="jpeg")||(ext == "png"))
  	    ...
  	    {
	 	MyAlert('Формат файла должен быть JPG или  PNG !', 'Ошибка');
	 	   return false;	
	 }
    	}
  //
...
function prepareUploadCopyMal(event)
 ...
 var options = {
	...
	return false;	
     ...
     if($('input#files').val() == '')
	 {  
	MyAlert('Копия паспорта обязательно!', 'Ошибка');
	return false;	
	}
	 if($('.error').length > 0)
        {
           ...
           else
{
   ...
   //    
      ...
      if(window.open(res, '_blank') == null)
        {
      MyAlert("Вашем интернет browser заблокирована всплывающие окна. \nТолько можно загрузить!","Уведомление");
