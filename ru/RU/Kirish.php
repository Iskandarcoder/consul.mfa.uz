...
<html>
	<head>
		...
		<!--<title>Kirish va ro`yhatga turish</title>-->
   ...
   <!-- форма входа -->
	<div id="login">     
				<form class="userform" id="login-form">
					<p class="fieldset">
						<label class="image-replace username" for="signin-nomi">Имя входа</label>
						<input class="full-width has-padding has-border has-error" id="signin-nomi" type="text" placeholder="Имя входа">
						<span class="error-message"></span>
					</p>
                  	         <p class="fieldset">
						<label class="image-replace password" for="signin-password">Пароль</label>
						<input class="full-width has-padding has-border has-error" id="signin-password" type="password"  placeholder="Пароль">
	 ...
	 <!-- форма регистрации -->
		<div id="signup">
				<form class="userform" id="signup-form">
					<p class="fieldset">
						<label class="image-replace username" for="signup-username">Имя входа</label>
		<input class="full-width has-padding has-border has-error" id="signup-username"  maxlength="20"  "text" placeholder="Имя входа">
						<span class="error-message"></span>
					</p>
                                             <p class="fieldset">
						<label class="image-replace password" for="signup-password">Пароль</label>
						<input class="full-width has-padding has-border has-error" id="signup-password" type="text"  placeholder="Пароль">
						<!--<a href="#0" class="hide-password">Berkitish</a>-->
						<span class="error-message"></span>
					</p>
					         <p class="fieldset">
						<label class="image-replace password" for="signup-password">Подтверждение пароля</label>
						<input class="full-width has-padding has-border has-error" id="verif-password" type="text"  placeholder="Подтверждение пароля">
						<span class="error-message"></span>
					</p>
					<p class="fieldset">
						<label class="image-replace email" for="signup-email">E-mail</label>
						<input class="full-width has-padding has-border " id="signup-email"    type="text" placeholder="E-mail"  >
						<span class="error-message">Ошибка в адресе почты!</span>
					</p>
			                   <p class="fieldset">
						<label class="image-replace username" for="signup-ism">Имя</label>
			<input class="full-width has-padding has-border has-error" id="signup-ism" maxlength="20"  type="text" placeholder="Имя">
						<span class="error-message"></span>
					</p>
								<p class="fieldset">
						<label class="image-replace username" for="signup-fam">Фамилия</label>
			<input class="full-width has-padding has-border has-error" id="signup-fam" maxlength="20"  type="text" placeholder="Фамилия">
						<span class="error-message"></span>
					</p>
...
<!-- форма восстановления пароля -->
	...
	<p class="cd-form-message">Забыли пароль? Пожалуйста, введите адрес своей электронной почты. Вы получите ссылку, чтобы создать новый пароль.</p>
			...
			<a href="#0" class="close-form">Закрыть</a>
...
//Форма входа
 ...
 //После получения ответа
      ...
      if(res == -1)
        {  
           MyAlert('Вы ввели ошибочные данные!','Ошибка');
          return;
          }
	    if(res == 0)
                window.history.back();
 ...
 //После получения ответа
      ...
      if(res == -1)
        {  
           MyAlert('Пользователь с таким именем входа зарегистрирован!','Ошибка');
          return;
          }
        if(res == -2)
        {
        MyAlert('Пользователь с таким e-mail зарегистрирован!','Ошибка');
          return;	
	}
	    if(res == 0)
                 MyAlert('Вы успешно зарегистрировались!','Информация');
	           window.history.back();
  	   	  return true;
  } 
//////  
  function proverka_text(id_pole_text)
  {
	     if(id_pole_text.val() == '')
	 {   
                id_pole_text.next('span').html('Это обязательное поле!').addClass('is-visible');
	...
	else
	   {
	 	  var pp = /^[а-яА-ЯёЁa-zA-Z][а-яА-ЯёЁa-zA-Z0-9-_\.]{1,20}$/;
	 	   if(!pp.test(id_pole_text.val()))
                     {
		 	id_pole_text.next('span').html('Первая должна быть буквой, остальные могут быть буква, число и подчеркивание!').addClass('is-visible');
	                 id_pole_text.removeClass('not_error').addClass('has-error');	
	                 return false;
		  }       
		  else
  ...
  function proverka_password(id_pole_password)
  {
	 ...
	 {   
                id_pole_password.next('span').html('Пароль обязательное роле!').addClass('is-visible');
	...
	else
	   {
	 	//  var pp = /(?=^.{6,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/;
	 	   if(id_pole_password.val().length<6)
                     {
		 	id_pole_password.next('span').html('Должна быть не менее 6 символов !').addClass('is-visible');
	                 id_pole_password.removeClass('not_error').addClass('has-error');	
	                 return false;
		  }       
		  else
