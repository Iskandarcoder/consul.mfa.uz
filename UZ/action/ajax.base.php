...
switch ($_POST['action']){
        ...
        case "showRegionForInsert":
  		 $lSql="SELECT sp_name04, sp_id FROM sp_district WHERE sp_region=".$_POST['id_country']." ORDER BY 1";
        ...
        case "showCityForInsert":
               $lSql="SELECT sp_name04, sp_id FROM sp_place WHERE sp_district=".$_POST['id_region']." ORDER BY 1";
				$select = mysql_query($lSql);
        ...
        case "showStreetForInsert":
               $lSql="SELECT sp_name04, sp_id FROM sp_street WHERE sp_district=".$_POST['id_place']." ORDER BY 1";
				$select = mysql_query($lSql);
				echo '<option value=0>----- Tanlanmagan -------- </option>';
                 while(list($sp_name04,$sp_id) = mysql_fetch_row($select))
               		echo "<option value='".$sp_id."'>$sp_name04</option>";
      ...
      case "showCountryLatInsert":
               $lSql="SELECT sp_name04, sp_id FROM sp_country WHERE sp_id=".$_POST['id_place']." ORDER BY 1";
				$select = mysql_query($lSql);
                 while(list($sp_name04,$sp_id) = mysql_fetch_row($select))
		              echo $sp_name04;
                ...
                break;
       case "showRegionLat":
               $lSql="SELECT sp_name04, sp_id FROM sp_region WHERE sp_id=".$_POST['id_place']." ORDER BY 1";
				$select = mysql_query($lSql);
                 while(list($sp_name04,$sp_id) = mysql_fetch_row($select))
		              echo $sp_name04;
