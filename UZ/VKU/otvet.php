...
if($x>1)
{
   echo "ID simvollarini ko`proq kiritib ko`ring!";
   exit;
}
  ...
  if($x==1)
{
  ...
  {
$b = $row['status'];    
  	$b = $b."<p class='otvet_o'>#Hurmatli &nbsp".$fam.'  '.$ism."</p>";
  ...
  if($row['status']=='6')
  {
  $b = "6#Hurmatli &nbsp".$fam.'  '.$ism."<span><br/> Sizning so`rovnomangiz tekshiruvdan o`tmadi!</span></p>";
  $info = "Konsul javobi:<br />".$row['add_info'];
...
if($x>0)
{
  ...
  // 
  $pdf->setSourceFile('../VKU/Kart_lat.pdf');
 ...
 //
  ...
  $elchixona = iconv("UTF-8","CP1251",$row["elchihona_qn_uz"]);
  $elchixona=wordwrap($elchixona,250,"\n");
	$pdf->SetXY(33,70);
	$pdf-> Cell(250,8,$elchixona,0,0,'L',0);
  
    $pdf->SetFontSize(8);  
  $elchixona = $row["ish_vaqti_uz"];
  $elchixona=wordwrap($elchixona,250,"\n");
	$pdf->SetXY(33,100);
	$pdf-> Cell(250,8,$elchixona,0,0,'L',0);
    
  $elchixona = $row["ish_vaqti_c_uz"];
  $elchixona=wordwrap($elchixona,250,"\n");
	$pdf->SetXY(33,110);
	$pdf-> Cell(250,8,$elchixona,0,0,'L',0);
  ...
  $pdf->SetFontSize(9);
  $elchixona = $row["elchihona_tn_uz"]; 
  	$elchixona=wordwrap($elchixona,250,"\n");
	$pdf->SetXY(310,97);
	$pdf-> Cell(250,8,$elchixona,0,0,'C',0);
    
    $elchixona = $row["elchihona_tn1_uz"];
 ...
 $pdf->closeParsers();
 print_r( "7/uz/uz/downloads/".$Sfile);
}
  	//unlink($SPathfile);
exit;
    }
    ...
    echo  "#<p class='otvet'>Bu ID ga bazadan ma`lumot topilmadi.<br/>Xato kiritilgan! </p>";
 }
?>
